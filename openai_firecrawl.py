...
class ModelPricing(BaseModel):
    check_point: str = Field(..., description="Name of the model")
...
async def scrape_openai_pricing_markdown():
...
def main():
    ...
    asyncio.run(scrape_openai_pricing_markdown())
    ...
    print(f"Extracting pricing for models: {openai_models}")
    openai_df = extract_pricing_from_text(page_content, openai_models, provider)

    if openai_df.empty:
        print("No pricing data extracted. Please check the markdown text and extraction logic.")
        return

    openai_df["need_to_check"] = openai_df.apply(lambda row: needs_check(row, ground_truth), axis=1)

    print("\nFinal Results:")
    print(openai_df.to_markdown(index=False))

    needs_check_count = openai_df["need_to_check"].sum()
    print(f"\nSummary: {needs_check_count} out of {len(openai_df)} models need manual verification.")

    openai_df.to_csv("openai_pricing_results.csv", index=False)
    print("Results saved to openai_pricing_results.csv")
